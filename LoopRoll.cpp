...
HRESULT VDJ_API LoopRollPlugin::OnStart()
{
    ...
    loopStart_ =
        std::max(
            0,
            (currentSample / loopSamples_) * loopSamples_
        );
...
short* VDJ_API LoopRollPlugin::OnGetSongBuffer(
    int pos,
    int nb)
{
    if (nb <= 0)
    ...
    while (processed < nb)
    {
        const int absolutePosition =
            pos + processed;

        int relative =
            (absolutePosition - loopStart_) %
            loopSamples_;
